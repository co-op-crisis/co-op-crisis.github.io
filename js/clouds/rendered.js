...
(function () {
  ...
  for (; i < number; i++) {if (window.CP.shouldStopExecution(0)) break;
  ...
  function bestCandidateSampler(width, height, numCandidates) {
    ...
    function getSample() {
      ...
      if (samples.length < 1) {
        ...
        for (; i < numCandidates; i++) {if (window.CP.shouldStopExecution(3)) break;
          c = [Math.random() * width, Math.random() * height];
  ...
  cloudImg.addEventListener('load', function () {
    ...
    while (i--) {if (window.CP.shouldStopExecution(5)) break;
